...
func NewGraphSet(rules map[string]*RuleSet, main string, target string, updated map[string]bool) (*GraphSet, error) {
	...
	return nil, fmt.Errorf("ruleset for directory not found: [%s]", main)
}
...
func NewGraph(rs *RuleSet, target, dir string, gs *GraphSet, updated map[string]bool) (g *Graph, err error) {
	...
	g.base, err = g.resolveTargetInterGraph(target, visits, gs, updated)
	if err != nil {
		return g, err
	}
...
func (g *Graph) resolveTargetInterGraph(prereq string, visits []int, gs *GraphSet, updated map[string]bool) (*node, error) {
	node, err := g.resolveTargetInDir(prereq, gs, updated)
	if err == nil {
		return node, nil
	}
	node, err = g.resolveTarget(prereq, visits, gs, updated)
	if err == nil {
		return node, nil
	}
...
func (g *Graph) resolveTargetInDir(prereq string, gs *GraphSet, updated map[string]bool) (*node, error) {
	// first attempt to resolve the target using the ruleset for its directory
	dir := filepath.Dir(prereq)
	name := filepath.Base(prereq)
	if dir != "." {
		if rs, ok := gs.rules[dir]; ok {
			// if this target uses a separate ruleset, create a subgraph and use
			// that to resolve the target.
			subg, err := NewGraph(rs, name, pathJoin(g.dir, dir), gs, updated)
			if err != nil {
				return nil, err
			}
			return subg.base, nil
	...
	return nil, fmt.Errorf("no buildset for: %s", dir)
}
...
func (g *Graph) resolveTarget(prereq string, visits []int, gs *GraphSet, updated map[string]bool) (*node, error) {
		...
		for mi := len(g.rs.metaRules) - 1; mi >= 0; mi-- {
			...
			if sub, pat := mr.Match(target); sub != nil {
				...
				for _, p := range metarule.prereqs {
					_, err := g.resolveTargetInterGraph(p, visits, gs, updated)
					if err != nil {
						failed = true
						break
					}
	...
	for _, p := range n.rule.prereqs {
		pn, err := g.resolveTargetInterGraph(p, visits, gs, updated)
		if err != nil {
