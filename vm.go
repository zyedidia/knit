...
func (vm *LuaVM) pkgknit() *lua.LTable {
	...
	vm.L.SetField(pkg, "prefix", luar.New(vm.L, func(in []string, prefix string) *lua.LTable {
	...
	vm.L.SetField(pkg, "suffix", luar.New(vm.L, func(in []string, suffix string) *lua.LTable {
		s := make([]string, 0, len(in))
		for _, v := range in {
			s = append(s, v+suffix)
		}
